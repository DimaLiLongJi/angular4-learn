...
let getDetail = (() => {
  var _ref = _asyncToGenerator(function* (pms) {
    const params = {
      method: 'get',
    ...
    let result;
    try {
      result = yield cfAdminService.execRpWithOptions(params);
    } catch (e) {
    ...
    if (result.locations && result.locations.length > 0) {
      result.locationText = result.locations.map(function (l) {
        return l.name;
      }).join('、');
    }
...
function getOpportunityList(pms) {
  ...
  return cfAdminService.execRpWithOptions(params).then(result => {
    const opps = [];
