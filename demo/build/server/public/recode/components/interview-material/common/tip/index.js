...
var Tip = function (_Component) {
  ...
  function Tip(props) {
    ...
    _this.download = function (tip, event) {
      // 下载权限
      ...
      tip.download = _this.context.config.CF_FILE_BASE_URL + '/' + tip.fileId + '?originalName=' + encodeURIComponent(tip.fileName) + '&download=1';
      window.location.href = tip.download;
      _this.setState({
        downloadCount: _this.state.downloadCount + 1
      });
      _this.props.countDownloadMaterial(tip.id);
    };

    _this.gotoPreview = function (tip) {
      if (!tip) return;
      _this.props.countViewMaterial(tip.id);
      _this.setState({
        viewCount: _this.state.viewCount + 1
      });
      var pathUrl = '/materials/' + tip.fileId + '/preview?fileName=' + encodeURIComponent(tip.fileName) + '&hd=1';
      window.open(pathUrl, '_blank');
    };

    _this.state = _extends({}, _this.props.tip);
    return _this;
  }
...
Tip.propTypes = {
  tip: _propTypes2.default.object,
...
exports.default = (0, _reactRedux.connect)(mapStateToProps, mapDispatchToProps)(Tip);