...
router.get('/danmu', (req, res) => {
  danmuService.getDanmuData(req.query).then(result => {
    res.json(result);
  }).catch(err => {
...
router.get('/danmu/customize', (req, res) => {
  ...
  danmuService.getCustomizedDanmuData(req.query).then(result => {
    res.json(result);
  }).catch(err => {
    err.message = err.message || '获取danmu失败';
    logger.debug('get danmu list failed, err is', err, 'params is', req.params);
...
router.get('/danmu/qa', (req, res) => {
  danmuService.getQADanmuData(req.query).then(result => {
    res.json(result);
  }).catch(err => {
