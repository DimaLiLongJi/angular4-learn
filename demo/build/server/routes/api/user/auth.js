...
router.get('/auth/user_info', (req, res) => {
  authService.getUserInfo(req.query).then((() => {
...
router.get('/auth/check_login', (req, res) => {
  return authService.getUserInfo(req.query).then((() => {
    var _ref2 = _asyncToGenerator(function* (result) {
      if (result && result.id) {
        ...
        const wechatToken = yield cipherService.createToken(tokenParams);
        console.log('wechatTokenwechatTokenwechatToken', wechatToken);
        res.cookie('wechatToken', wechatToken, {
          // domain: config.domain,
          maxAge: 15 * 24 * 60 * 60 * 1000 // 15d
        });
...
router.put('/auth/pw_recover', verifyCode, (req, res) => {
  const params = req.body;
  return authService.recoverPW(params).then(result => {
    if (!result.err) {
      console.log('destroy session!!!');
      req.session.destroy();
      res.clearCookie('verifyCode');
...
router.get('/auth/:userId(\\d+)/refresh_token', (req, res) => {
  authService.getUserInfo({ id: req.params.userId }).then((() => {
...
router.get('/auth/captcha', (req, res) => {
  ...
  return authService.getCaptcha().then(result => {
    if (result) {
...
router.post('/auth/send_verify_code_sms', verifyCaptcha, (req, res) => {
  ...
  return authService.sendVerifyCodeSms(params).then(result => {
...
router.get('/auth/mobile_check', (req, res) => {
  ...
  return authService.checkMobile(params).then(result => res.send(result)).catch(error => {
    const errorObj = {
      msg: '检查手机号失败',
      error
    };
...
router.post('/auth/check_verify_code', verifyCode, (req, res) => res.send({ result: 1 }));

function verifyCaptcha(req, res, next) {
  if (!req.cookies.captcha || !req.body.captcha || req.body.captcha.toUpperCase() !== req.cookies.captcha) {
    const errorObj = {
      msg: '图形验证码校验失败'
    };
