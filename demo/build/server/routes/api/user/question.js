...
router.post('/users/:userId(\\d+)/questions', (req, res) => {
  userQuestionService.create(req.params.userId, req.body).then(result => {
    res.json(result);
  }).catch(err => {
...
router.get('/users/:userId(\\d+)/questions/checkCount', (req, res) => {
  userQuestionService.checkCount(req.params.userId).then(result => {
    res.json(result);
  }).catch(err => {
...
router.get('/users/questions', (req, res) => {
  ...
  userQuestionService.getList(params).then(result => {
    res.json(result);
  }).catch(err => {
...
router.get('/users/:userId(\\d+)/questions', (req, res) => {
  ...
  userQuestionService.getUserQuestions(params).then(result => {
    res.json(result);
  }).catch(err => {
...
router.get('/users/:userId(\\d+)/questions/check_read', (req, res) => {
  ...
  userQuestionService.checkUserUnread(params).then(result => {
    res.json(result);
  }).catch(err => {
    err.message = err.message || 'get question detail failed';
    logger.debug('get question detail failed, err is', err, 'params is', req.params);
...
router.get('/users/questions/customize', (req, res) => {
  ...
  userQuestionService.getCustomizedQuestions(params).then(result => {
    res.json(result);
  }).catch(err => {
    err.message = err.message || '获取问题列表失败';
    logger.debug('create user attachment failed, err is', err, 'params is', req.params);
...
router.get('/users/questions/:id(\\d+)', (req, res) => {
  ...
  userQuestionService.getDetail(params).then(result => {
    res.json(result);
  }).catch(err => {
    err.message = err.message || 'get question detail failed';
    logger.debug('get question detail failed, err is', err, 'params is', req.params);
