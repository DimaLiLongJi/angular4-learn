...
router.get('/users/:userId(\\d+)/applications/latest', (req, res) => {
  userApplicationService.getUserLastApplication(req.params.userId).then(result => {
    res.json(result);
  }).catch(err => {
...
router.post('/users/:userId(\\d+)/applications', (req, res) => {
  ...
  userApplicationService.create(req.params.userId, req.body).then(result => {
    res.json(result);
  }).catch(err => {
...
router.get('/users/:userId(\\d+)/applications', (req, res) => {
  userApplicationService.getUserApplicationList(req.params.userId, req.query).then(result => {
    res.json(result);
  }).catch(err => {
...
router.get('/users/:userId(\\d+)/applications/:opportunityId/available', (req, res) => {
  userApplicationService.checkApplyAvailable(req.params).then(result => {
    res.json(result);
  }).catch(err => {
