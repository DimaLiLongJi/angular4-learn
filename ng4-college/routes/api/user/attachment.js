...
router.get('/users/:userId(\\d+)/attachments', (req, res) => {
  userAttachmentService.getUserAttachments(req.params.userId)
    .then((result) => {
      res.json(result);
    })
    .catch((err) => {
...
router.post('/users/:userId(\\d+)/attachments', (req, res) => {
  userAttachmentService.create(req.params.userId, req.body)
    .then((result) => {
      res.json(result);
    })
    .catch((err) => {
...
router.delete('/users/:userId(\\d+)/attachments/:attachmentId(\\d+)', (req, res) => {
  userAttachmentService.destroy(req.params.userId, req.params.attachmentId)
    .then((result) => {
      res.json(result);
    })
    .catch((err) => {
