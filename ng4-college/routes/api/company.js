...
router.get('/companies/:companyId(\\d+)', (req, res) => {
  ...
  companyService.getDetail(id, req.query)
    .then((result) => {
      res.json(result);
    })
    .catch((err) => {
...
router.get('/recommend_companies', (req, res) => {
  companyService.getRecommendList()
    .then((result) => {
      res.json(result);
    })
    .catch((err) => {
...
router.get('/companies/university_tour', (req, res) => {
  companyService.getUTCompanyList()
    .then((result) => {
      res.json(result);
    })
    .catch((err) => {
...
router.get('/similar_companies/:companyId(\\d+)', (req, res) => {
  companyService.getSimilarList(req.params.companyId)
    .then((result) => {
      res.json(result);
    })
    .catch((err) => {
...
router.get('/companies/:companyId(\\d+)/university_tour', (req, res) => {
  companyService.getDetail(req.params.companyId, { getUTOpp: 1, })
    .then((result) => {
